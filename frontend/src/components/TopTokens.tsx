...
export default function TopTokens() {
  // add isProfit and calculate difference percent
  data = data.map(token => {
    const first = token.data[0]
    const last = token.data[token.data.length - 1]

    token.isProfit = first.amt < last.amt

    const diff = last.amt - first.amt
    token.percentDiff = (diff / first.amt) * 100
